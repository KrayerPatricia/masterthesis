...
Food_contents_summary %>%
  ...
  summarise(AnimalProd = sum(sum[Commodities == "All animal based Commodities"], na.rm = TRUE), 
            CropProd = sum(sum[Commodities == "All crop based Commodities"], na.rm = TRUE),
            PermProd = sum(sum[Commodities == "All perm Crop Commodities"], na.rm = TRUE)) %>%
...
Food_contents_summary %>%
  filter(Contents == "Calories (kcal)",
         Commodities %in% c("All animal based Commodities", "All crop based Commodities", "All perm Crop Commodities",  "All Commodities")) %>%
  group_by(Contents, Scenarios,ProductionSystems) %>% #
  summarise(AnimalProd = sum(sum[Commodities == "All animal based Commodities"], na.rm = TRUE), 
            CropProd = sum(sum[Commodities == "All crop based Commodities"], na.rm = TRUE),
            PermProd = sum(sum[Commodities == "All perm Crop Commodities"], na.rm = TRUE),
            AllProd = sum(sum[Commodities == "All Commodities"], na.rm = TRUE)) %>%
  mutate(CropWoPerm = CropProd - PermProd, Others = AllProd - AnimalProd- CropProd) %>%
  select(-AllProd) %>%
  filter(Scenarios %in% c("BaselineDerived", "Baseline", "FOFA_BAU_2050") |
    !(Scenarios %in% c("BaselineDerived", "Baseline", "FOFA_BAU_2050")) & ProductionSystems != "AllProdSyst") %>%
  mutate(Pop = ifelse(Scenarios %in% c("BaselineDerived", "Baseline"), Population_summary$sum[Population_summary$Scenarios == "BaselineDerived"], 
                      Population_summary$sum[Population_summary$Scenarios == "FOFA_BAU_2050"])) %>%
  mutate(AnimalProd_PP = AnimalProd / Pop / 365, CropProd_PP = CropWoPerm / Pop / 365, PermProd_PP = PermProd / 365/Pop, OtherProd_PP = Others /365/Pop) %>%
  select(1:3,10:13)  %>%
  pivot_longer(4:7, names_to = "Organic_Vegan_Results_Indicators", values_to = "sum_per_capita_day") %>%
  ggplot(aes(x = Scenarios, y = sum_per_capita_day, fill = Organic_Vegan_Results_Indicators)) +
  geom_bar(stat = "identity", position = "stack") +
  ggtitle("kcal per capita and day")+
  theme(axis.text.x = element_text(angle = 45, hjust=1)) 
...
#   summarise(sum_global = sum(Val))%>% #sum over all countries
#   mutate(Pop  = ifelse(Scenarios %in% c("BaselineDerived", "Baseline"), Population_summary$sum[Population_summary$Scenarios == "BaselineDerived"], 
#                             Population_summary$sum[Population_summary$Scenarios == "FOFA_BAU_2050"])) %>% # add population numbers
#   mutate(sum_global_PP = sum_global / Pop / 365) %>% #add global production per capita and day
#   group_by(Scenarios)  %>%
#   mutate(Sum_total = sum_global["All Commodities"])
# #  mutate(sum_global_AC = sum_global[Commodities == "All animal based Commodities"] + sum_global[Commodities == "All crop based Commodities"] ) %>%
# #  mutate(sum_global_AC_PP = sum_global_AC / Pop /365)-> test
#   
  ...
  # 1) plot nutrients
   output$plotFoodAvailability <- renderPlot({
    ...
    Food_contents_summary %>%
      ...
      select(1:3,10:13) %>%
      pivot_longer(4:7, names_to = "Commodities2", values_to = "Sum_per_capita_day") %>%
      mutate(cat = ifelse(Scenarios %in% c("Baseline", "BaselineDerived", "FOFA_BAU_2050"), "Reference",
                          ifelse(Scenarios %in% c("FOFA_2050_VeganBAU", "FOFA_2050_VeganBAU_NoFreeAreaUse", "FOFA_2050_VeganOptimized_conv"), "Vegan-Conventional",
                                 "Vegan-Organic"))) %>%
      #remove all commodities (error in data)
      filter(Commodities2 != "OtherProd_PP") %>%
